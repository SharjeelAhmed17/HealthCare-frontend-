...
import CS_map from './CS_map';
import { useDispatch, useSelector } from 'react-redux';
import { Hop_OPDSelect, Hop_OPD} from '../features/Hop_OPCount'
import { Hop_ICUSelect, Hop_ICU} from '../features/Hop_ICUCount'
import { Hop_DischargedSelect, Hop_Discharged} from '../features/Hop_DischargedCount'
...
export default function CostumerService() {
  ...
  const OPDcounts = useSelector(Hop_OPDSelect) 
  ...
  const handleCSMap = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:3002/cs_patientList', data, { headers, withCredentials : true });
      ...
      if (response.status === 200) {
        ...
        dispatch(Hop_OPD(OPDcount))
        dispatch(Hop_ICU(ICUcount))
        dispatch(Hop_Discharged(dischargedcount))
      }
    } catch (error) {
      console.log(error);
      if (error.response) {
      ...
      <ToastContainer/>
        <section className="absolute w-full h-full flex flex-row text-center">
            <sideBar className= ' basis-1/6 bg-gray-900 text-white h-screen '>
                    ...
                    <hr />
                    <Link to={'/add_patient'}>
                    <button className="bg-gray-900 text-white active:bg-gray-700 text-sm font-bold uppercase px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-full inline-flex items-center justify-center text-xs">
                    ...
                    <button
                      className="bg-red-200 text-black active:bg-gray-700 text-sm font-bold uppercase px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-full inline-flex items-center justify-center text-xs">
                        ...
                        <span className='ml-3'> Hospital OPD ({OPDcounts})</span>
                    </button>
                    <hr />
                    <button
                      className="bg-red-500 text-black active:bg-gray-700 text-sm font-bold uppercase px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-full inline-flex items-center justify-center text-xs">
                        ...
                        <span className='ml-3'> Hospital ICU ({ICUcounts}) </span>
                    </button>
                    <hr />
                    <button
                      className="bg-lime-500 text-black active:bg-gray-700 text-sm font-bold uppercase px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-full inline-flex items-center justify-center text-xs">
                        ...
                        <span className='ml-3'> Hospital Discharged ({Dischargedcounts}) </span>
                    </button>
                    <hr />
                    <button
                    onClick={signOut} 
            ...
            <div className=" basis-5/6 container mt-5 overflow-y-auto h-90">
                ...
                <div className="  px-4">
                    ...
                    <div className=" rounded-t mt-6 mb-0 px-6 py-6">
                        ...
                        {OPD.length>0?
                            OPD.map((item)=>{
                              return <CS_map id={item.id} fullName={item.fullName} gender={item.gender} age={item.age} status={item.status} doctor={item.doctor} costumerService={item.costumerService} handleCSMap={handleCSMap} />
                            })
                            :<div></div>}
                        </div>
                    </div>
